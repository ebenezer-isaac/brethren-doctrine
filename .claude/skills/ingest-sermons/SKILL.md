...
- They reference but never override Levels 1 to 3 (interlinear, translations).
...
Chunk semantically, not by length. A chunk should be ONE coherent unit: a single argument, a single quoted verse with its commentary, a single illustration. Aim for 100 to 600 words per chunk; smaller is fine for short claims.

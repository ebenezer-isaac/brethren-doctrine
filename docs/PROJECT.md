...
A tradition-neutral lexical-philological floor for the 231 questions in `questions.json`. Each question is settled by **apparatus + interlinear + concordance**. Counter-witness traditions are recorded in `evidence/<id>.json` as diagnostic information showing how each major lineage reads the same lexical text; they do not vote on any verdict field. Confessions and Brethren teaching notes are NOT in this pipeline; they live in pipeline B.

Output: `baseline.json` (231 × 13-field answers) + `evidence/<id>.json` (per-question audit trail).
...
### Tier 2: Semantic + Graph layer (BUILT including concordance; Bible-text translations PENDING)
...
- **Concordance ingestion complete**: Neo4j carries 17,003 lemmas, 447,700 tokens, 34,128 verses, 600,364 OpenBible cross-references, 591,039 TSK cross-references. TAHOT + TAGNT + OSHB + OpenBible + TSK ingested via idempotent loaders in [ingest/adapters/concordance_loader.py](../ingest/adapters/concordance_loader.py). See [CONCORDANCE.md](CONCORDANCE.md).
...
**Cult-marker bar.** Two conditions, both canonical: (i) `would_die_for=true` entailment; (ii) apparatus + interlinear + concordance demonstrate the doctrine canonically (pan-canonical lexical reading, not single-passage). **Even Trinity is not exempt.** Lineage agreement does NOT grant or withhold cult-marker status; Scripture's clarity drives the bar. The Brethren-on-trial discipline is preserved through canonical demonstration: most Brethren distinctives lack pan-canonical lexical support and fail honestly. See [ANSWER_SCHEMA.md](ANSWER_SCHEMA.md) §Cult-marker bar.
...
| Phase | Scope | Status |
|---|---|---|
| **1. Architecture** | All docs/, tools/, schemas, README, memory aligned to the tradition-neutral lexical-philological architecture with apparatus + interlinear + concordance as the judging panel and counter-witness as diagnostic research panel. Cult-marker bar driven by canonical demonstration. | **Complete** |
| **1.5 Concordance ingestion** | Idempotent loaders in `ingest/adapters/concordance_loader.py` populated Neo4j with TAHOT + TAGNT + OSHB + OpenBible + TSK. Live counts: 17,003 lemmas, 447,700 tokens, 34,128 verses, 600,364 OpenBible cross-refs, 591,039 TSK cross-refs. | **Complete** |
| **1.6 Question-bank stem audit and tier reset** | Phase B Opus audit applied 12 splits, 11 neutralizations, 23 tier moves, 9 historical_consensus refinements, 8 K2 catalog moves. Tier abolished from `questions.json` entirely as a follow-up. Standing now inferred from the answer's boolean pattern. | **Complete** |
| **2. Inferred-baseline orchestrator run** | Run `tools/baseline_orchestrator.py` to produce `baseline.json` + `evidence/*.json` for all 231 questions. Recommended model: Opus (canonical-demonstration synthesis benefits from Opus reasoning quality on every question). Gated on KPI verifier green-light + consistency-audit findings addressed. | Awaiting gates |
...
| **3. Trusted-elder questionnaire round** | Distribute the 231-question form to trusted churches/elders; collect `responses/*.json`. Each respondent's answers ingest via `verdict_loader load-baseline --viewpoint individual:<id>` so their lens joins the graph alongside the inferred baseline. | Pending |
| **4. Consolidation** | Collate baseline + responses + research into `consolidated.json`; ingest as `viewpoint='consolidated'`. The same 231 question nodes now have multiple verdict edges (inferred + per-respondent + consolidated), each queryable. | Pending |
...
- Tier 2 concordance + Bible-text ingestion: **complete** (Neo4j: 17,003 lemmas, 447,700 tokens, 34,128 verses, 600,364 OpenBible cross-refs, 591,039 TSK cross-refs).
...
- Inferred-baseline orchestrator run (phase 2): **gated on KPI verifier green-light + consistency-audit findings addressed**. (Phase 1.5 concordance ingestion and 1.6 stem audit + tier reset both complete.)
